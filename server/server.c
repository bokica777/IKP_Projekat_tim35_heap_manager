...
static void handle_command(SOCKET client, char *line)
{
    ...
    if (_stricmp(cmd, "ALLOC") == 0)
    {
        ...
        if (!job)
        {
            send_line(client, "ERR oom\n");
            return;
        }

        if (hm_results_put_pending(id, job) != 0)
        {
            HeapFree(GetProcessHeap(), 0, job);
            send_line(client, "ERR map_full\n");
            return;
        }
        if (hm_jobs_push(job) != 0)
        {
            hm_results_remove(id);
            HeapFree(GetProcessHeap(), 0, job);
            send_line(client, "ERR queue_full\n");
            return;
        }
    ...
    if (_stricmp(cmd, "FREE") == 0)
    {
        ...
        unsigned long long v = _strtoui64(s, NULL, 16);
    ...
    if (_stricmp(cmd, "POLL") == 0)
    {
        ...
        {
            char out[128];
            sprintf(out, "DONE 0x%llX\n", (unsigned long long)(uintptr_t)res);

            send_line(client, out);

            if (jobp)
                HeapFree(GetProcessHeap(), 0, jobp);
            hm_results_remove(id);
            return;
        }
...
int main(void)
{
    ...
    if (hm_jobs_init() != 0)
    {
        printf("hm_jobs_init FAILED\n");
        return 1;
    }
    if (hm_results_init() != 0)
    {
        printf("hm_results_init FAILED\n");
        return 1;
    }
    if (hm_pool_start(4) != 0)
    {
        printf("hm_pool_start FAILED\n");
        return 1;
    }
