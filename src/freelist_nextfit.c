...
void hm_freelist_insert(hm_block_header_t* b) {
    ...
    g_free_head = b;
    if (!g_free_tail) g_free_tail = b;

    if (!g_cursor) g_cursor = b;
}
...
void hm_freelist_remove(hm_block_header_t* b) {
    ...
    else g_free_tail = b->prev_free;

    if (g_cursor == b) g_cursor = b->next_free ? b->next_free : g_free_head;
...
hm_block_header_t* hm_nextfit_find(size_t need_total) {
    ...
    if (!g_cursor) g_cursor = g_free_head;
